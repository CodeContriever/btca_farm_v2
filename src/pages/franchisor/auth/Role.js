...
const setRoleAndNavigate =  useCallback(async (userId) => {
  try {
    ...
    if (response.status === 200) {
      ...
      navigate("/franchisor/edit_profile"); // Navigate to the farmer page
...
useEffect(() => {
  ...
  disabled  
>
  <option value="farmer">Franchisor</option>
</select>
                </div>
                ...
                <div className="py-4 flex flex-row gap-8">
                  <span className="text-gray-500">
                    Already Registered?
                    <Link className="text-red-500 ml-2" to="/franchisor/signin">
                      Login Now
                    </Link>
                  </span>
                </div>
