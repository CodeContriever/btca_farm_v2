...
import { selectSuperAdminProfileData } from '../../store/superAdmin/Profile';
...
function HamburgerMenu({ activeButton, handleHamburgerClick, onNavigate }) {
 ...
 <li onClick={() => handleHamburgerClick('menu')}>
             ...
             <li onClick={() => handleHamburgerClick('menu')}>
              <button className="cursor-pointer text-gray-800 hover:text-[#A020F0] focus:text-[#A020F0]" onClick={() => onNavigate('approvals')}>Approvals</button>
            </li>
...
function UserProfileDropdown() {
  ...
  const superAdminProfileData = useSelector((state) => state.superAdminProfile.superAdminProfileData);

  const { fullname, email, role } = superAdminProfileData?.data || {};
  ...
  return (
    <div className="relative flex flex-col justify-center items-center" tabIndex="0">
      ...
      >
        ...
        {superAdminProfileData?.avatarUrl ? (
          <img
            src={superAdminProfileData.avatarUrl}
            alt="User Avatar"
