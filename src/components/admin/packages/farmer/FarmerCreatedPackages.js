...
const FarmerCreatedPackages = () => {
  ...
  useEffect(() => {
    ...
    const fetchPackages = async () => {
      try {
        ...
        setFarmerCreatedPackages(response.data.data); // Access 'data' property if needed
      } catch (error) {
 ...
 const handleEditClick = (index) => {
  setEditingPackage({ ...farmerCreatedPackages[index] });
};
...
const handleEditPackage = async (updatedPackage) => {
  try {
    const response = await axios.post(
      'https://api.afribook.world/package/editPackage',
      updatedPackage,
      {
        headers: {
    ...
    const fetchPackagesResponse = await axios.get('https://api.afribook.world/package/getPackages', {
      headers: {
        Authorization: `Bearer ${signinData?.accessToken || ''}`,
      },
    });
    ...
    setFarmerCreatedPackages(fetchPackagesResponse.data.data);
  ...
  const handleDeleteClick = (index) => {
    setPackageToDelete({ ...farmerCreatedPackages[index] });
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await axios.post(
        'https://api.afribook.world/package/deletePackage',
      ...
      setFarmerCreatedPackages(farmerCreatedPackages.filter((pkg) => pkg.packageId !== packageToDelete.packageId));
  ...
  return (
    <div>
      ...
      <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-4 lg:space-y-0 lg:space-x-2">
        ...
        {farmerCreatedPackages.map((pkg, index) => (
          <div
            key={index}
...
export default FarmerCreatedPackages;
