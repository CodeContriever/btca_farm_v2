...
const ResellerCreatedPackages = () => {
  ...
  useEffect(() => {
    ...
    const fetchPackages = async () => {
      try {
        ...
        setResellerCreatedPackages(response.data.data); // Access 'data' property if needed
      } catch (error) {
 ...
 const handleEditClick = (index) => {
  setEditingPackage({ ...resellerCreatedPackages[index] });
};
...
const handleEditPackage = async (updatedPackage) => {
  try {
    const response = await axios.post(
      'https://api.afribook.world/package/editResellerPackage',
      updatedPackage,
      {
        headers: {
    ...
    const fetchPackagesResponse = await axios.get('https://api.afribook.world/package/getResellerPackages', {
      headers: {
        Authorization: `Bearer ${signinData?.accessToken || ''}`,
      },
    });
    ...
    setResellerCreatedPackages(fetchPackagesResponse.data.data);
  ...
  const handleDeleteClick = (index) => {
    setPackageToDelete({ ...resellerCreatedPackages[index] });
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      setResellerCreatedPackages(resellerCreatedPackages.filter((pkg) => pkg.packageId !== packageToDelete.packageId));
  ...
  return (
    <div>
      ...
      <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-4 lg:space-y-0 lg:space-x-2">
        ...
        {resellerCreatedPackages.map((pkg, index) => (
          <div
            key={index}
...
export default ResellerCreatedPackages;
