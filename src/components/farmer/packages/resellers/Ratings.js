...
const ResellersRating = ({ resellerId }) => {
  const [rating, setRating] = useState(0);

  useEffect(() => {
    const fetchRating = async () => {
      try {
        const response = await axios.get(`https://api.afribook.world/franchisor/getRating/${resellerId}`);

        if (response.status === 200) {
          const data = response.data;
          console.log('Reseller rating fetch successful:', data);

          setRating(data.rating || 0); // Use 0 if the rating is not available
        } else {
          console.error('Error fetching reseller rating, please try again later.');
        }
      } catch (error) {
        console.error('Error fetching reseller rating:', error);
        // Handle the error, e.g., display an error message to the user
      }
    };

    // Fetch the rating when the component mounts or when resellerId changes
    if (resellerId) {
      fetchRating();
    }
  }, [resellerId]);
...
export default ResellersRating;
